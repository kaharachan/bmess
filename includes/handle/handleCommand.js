...
//// admin -pa /////
  ...
  const noleak = moment.tz("Asia/Ho_Chi_Minh").format("(DD/MM/YYYY)  (HH:mm:ss)");
      const apiUrl = 'http://toan-culi.tutoan205.repl.co/images/rem';
const numOfCalls = ["2"];
const array = [];
...
var nmt = res.data.data;
      const time = process.uptime(); 
      var anh = Math.floor(time / (60 * 60));
	var la = Math.floor((time % (60 * 60)) / 60);
	var toan = Math.floor(time % 60);
      const checker = stringSimilarity.findBestMatch(commandName, allCommandName);
      if (checker.bestMatch.rating >= 0.5) command = client.commands.get(checker.bestMatch.target);
      else return api.sendMessage({body: global.getText("handleCommand", "commandNotExist", checker.bestMatch.target, noleak, nmt,anh,la,toan,timeStart,), attachment:array} , event.threadID, event.messageID);
    }
