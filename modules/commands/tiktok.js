...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (args[0] == 'post') return runListUserPost(api, event, (await get(`https://caochungdat.me/docs/tiktok/post?unique_id=${args[1]}`)).data.data.videos, 6, 1, true, event.senderID);
  ...
  switch (type.toLowerCase()) {
   case "-i":
    case "info":
      ...
      try {
        axios.get(encodeURI(`https://www.nguyenmanh.name.vn/api/tikInfo?query=thuhoai2054&apikey=H3kKQ95i`)).then(async (res) => {
    ...
    case 'search':
      ...
      axios.get(`https://caochungdat.me/docs/tiktok/search?keywords=${keyword}`).then(async res => {
