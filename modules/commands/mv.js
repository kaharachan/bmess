...
module.exports.onLoad = () => {
    ...
    if (!fs.existsSync(dirMaterial + "mv.gif")) request("https://i.imgur.com/VcDvUXN.gif").pipe(fs.createWriteStream(dirMaterial + "mv.gif"));
}
...
`,attachment: fs.createReadStream(__dirname + `/noprefix/mv.gif`)}, event.threadID, event.messageID);
  ...
  if (money >= 100) {
  axios.get('https://toanyeuluv.sadboyyeu.repl.co/gai ').then(res => {
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://TPKTAO.last-namename.repl.co/image/vdsex').then(res => {
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://random.sadboyyeu.repl.co/video/anime').then(res => {
  var video = res.data.data;
  let callback = function () {
          api.sendMessage({
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://TPKTAO.last-namename.repl.co/image/vdmschil').then(res => {
  var video = res.data.url;
  let callback = function () {
          api.sendMessage({
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://TPKTAO.last-namename.repl.co/image/vdbautroi').then(res => {
  var video = res.data.url;
  let callback = function () {
          api.sendMessage({
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://TPKTAO.last-namename.repl.co/image/vddoraemon').then(res => {
  var video = res.data.url;
  let callback = function () {
          api.sendMessage({
