...
module.exports.handleEvent = async function ({ api, event, Users, Threads }) {
 ...
 if (time >= 22 || (time >= 0 && time < 5)) session = "khuye"  
   ...
   var icon1 = ["🙆‍♂️", "🙆‍♀️", "🙆", "🙇‍♂️", "🙇‍♀️", "🙇"];
   var icon2 = ["😴", "😪", "🥱", "🤐", "😑", "🙄", "💢"];
   var value = [`xin chào ${name}`, `bot xin chào ${name}`, `chao ${name}`, `hey xin chao ${name}`, `a nhô ${name}`, `xinchao ${name}`, `hêy xin chào ${name}`, `hí xin chào ${name}`];
